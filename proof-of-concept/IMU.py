...
class IMU(object):
    ...
    def gyro_scaled_x(self, x):
        return x * np.radians(self.gyro_gain_x)
    def gyro_scaled_y(self, x):
        return x * np.radians(self.gyro_gain_y)
    def gyro_scaled_z(self, x):
        return x * np.radians(self.gyro_gain_z)
    ...
    def drift_correction(self):
        ...
        self.omega_p = vector_scale(self.error_roll_pitch,self.kp_rollpitch*Accel_weight);

        self.scaled_omega_i = vector_scale(self.error_roll_pitch,self.ki_rollpitch*Accel_weight);
        ...
        self.scaled_omega_p = vector_scale(self.error_yaw,self.kp_yaw)
        self.omega_p = vector_add(self.omega_p,self.scaled_omega_p)

        self.scaled_omega_i = vector_scale(self.error_yaw,self.ki_yaw)
        self.omega_i = vector_add(self.omega_i,self.scaled_omega_i)
