...
def vector_cross(vectorOut, v1, v2):
    vectorOut[0]= (v1[1]*v2[2]) - (v1[2]*v2[1]);
    vectorOut[1]= (v1[2]*v2[0]) - (v1[0]*v2[2]);
    vectorOut[2]= (v1[0]*v2[1]) - (v1[1]*v2[0]);

def vector_scale(vectorOut, vectorIn, scale2):
    for c in xrange(3):
        vectorOut[c] = vectorIn[c]*scale2

def vector_add(vectorOut, vectorIn1, vectorIn2):
    for c in xrange(3):
...
class IMU(object):
    def __init__(self):
        ...
        self.sensor_sign = (1,1,1,-1,-1,-1,1,1,1) #Correct directions x,y,z - gyro, accelerometer, magnetometer
        
        self.gravity = 256

        self.gyro_gain_x = 0.07
        self.gyro_gain_y = 0.07
        self.gyro_gain_z = 0.07
    ...
    def gyro_scaled_x(self, x):
        return x * to_rad(self.gyro_gain_x)
    def gyro_scaled_y(self, x):
        return x * to_rad(self.gyro_gain_y)
    def gyro_scaled_z(self, x):
        return x * to_rad(self.gyro_gain_z)
    ...
    def print_data(self):
        print '% .2f % .2f % .2f' % (self.pitch, self.roll, self.yaw)
        #print '% .2f % .2f % .2f' % (self.dcm_matrix[2][0],
        #                             self.dcm_matrix[2][1],
        #                             self.dcm_matrix[2][2])
    ...
    def compass_heading(self):
        ...
        self.cmx = (self.mx - self.sensor_sign[6]*self.min_mx) / (self.max_mx - self.min_mx) - self.sensor_sign[6]*0.5
        self.cmy = (self.my - self.sensor_sign[7]*self.min_my) / (self.max_my - self.min_my) - self.sensor_sign[7]*0.5
        self.cmz = (self.mz - self.sensor_sign[8]*self.min_mz) / (self.max_mz - self.min_mz) - self.sensor_sign[8]*0.5
    ...
    def read_gyro(self):
        ...
        self.gx = self.sensor_sign[0] * (self.an[0] - self.an_offset[0])
        self.gy = self.sensor_sign[1] * (self.an[1] - self.an_offset[1])
        self.gz = self.sensor_sign[2] * (self.an[2] - self.an_offset[2])
    ...
    def read_accel(self):
        ...
        self.ax = self.sensor_sign[3] * (self.an[3] - self.an_offset[3])
        self.ay = self.sensor_sign[4] * (self.an[4] - self.an_offset[4])
        self.az = self.sensor_sign[5] * (self.an[5] - self.an_offset[5])
    
    def read_compass(self):
        self.mx = self.sensor_sign[6] * self.data_source[3]
        self.my = self.sensor_sign[7] * self.data_source[4]
        self.mz = self.sensor_sign[8] * self.data_source[5]
    ...
    def normalize(self):
        ...
        error= -vector_dot(self.dcm_matrix[0],self.dcm_matrix[1])*.5
        
        vector_scale(temporary[0], self.dcm_matrix[1], error)
        vector_scale(temporary[1], self.dcm_matrix[0], error)
        
        vector_add(temporary[0], temporary[0], self.dcm_matrix[0])
        vector_add(temporary[1], temporary[1], self.dcm_matrix[1])
        
        vector_cross(temporary[2],temporary[0],temporary[1])
        
        renorm= .5 *(3 - vector_dot(temporary[0],temporary[0]))
        vector_scale(self.dcm_matrix[0], temporary[0], renorm)
        
        renorm= .5 *(3 - vector_dot(temporary[1],temporary[1]))
        vector_scale(self.dcm_matrix[1], temporary[1], renorm)
        
        renorm= .5 *(3 - vector_dot(temporary[2],temporary[2]))
        vector_scale(self.dcm_matrix[2], temporary[2], renorm);
    ...
    def drift_correction(self):
        ...
        Accel_magnitude = Accel_magnitude / self.gravity
        ...
        vector_cross(self.error_roll_pitch,self.accel_vector,self.dcm_matrix[2])
        vector_scale(self.omega_p,self.error_roll_pitch,self.kp_rollpitch*Accel_weight);
        
        vector_scale(self.scaled_omega_i,self.error_roll_pitch,self.ki_rollpitch*Accel_weight);
        vector_add(self.omega_i,self.omega_i,self.scaled_omega_i);     
        ...
        vector_scale(self.error_yaw,self.dcm_matrix[2],errorCourse)
        
        vector_scale(self.scaled_omega_p,self.error_yaw,self.kp_yaw)
        vector_add(self.omega_p,self.omega_p,self.scaled_omega_p)
        
        vector_scale(self.scaled_omega_i,self.error_yaw,self.ki_yaw)
        vector_add(self.omega_i,self.omega_i,self.scaled_omega_i)
    ...
    def matrix_update(self):
        ...
        vector_add(self.omega, self.gyro_vector, self.omega_i) #adding proportional term
        vector_add(self.omega_vector, self.omega, self.omega_p) # //adding Integrator term
